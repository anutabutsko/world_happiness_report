...
covid <- covid %>%
  ...
  select(Country.Name, iso_code, date, `Daily ICU occupancy`, `Daily ICU occupancy per million`) %>%
  mutate(
    Year = year(date),
    month = month(date),
    day = day(date),
...
# by week
covid_summary <- covid %>%
  group_by(Country.Name, iso_code, Year, month, week_of_year) %>%
  summarize(
...
covid_summary <- covid_summary %>%
  group_by(Country.Name, iso_code, Year, month) %>%
  summarize(
...
#THEN by Year
covid_summary <- covid_summary %>%
  group_by(Country.Name, iso_code, Year) %>%
  summarize(
    Avg_Daily_ICU_Occupancy = mean(`Avg_Daily_ICU_Occupancy`, na.rm = TRUE),
    Avg_Daily_ICU_Occupancy_Per_Million = mean(`Avg_Daily_ICU_Occupancy_Per_Million`, na.rm = TRUE)
  )
...
#str(happydf)
happydf <- full_join(happydf, case, by = c("Country.Name", "Year"))
happydf$Year <- as.numeric(as.character(happydf$Year))
...
Export Data to CSV into Datasets
```{r}
...
Export Data to CSV into Datasets
```{r}
path <- here("DataAnalysis","Datasets","WorldHappinessReport.csv")
write.csv(happydf, path, row.names=FALSE)
```
