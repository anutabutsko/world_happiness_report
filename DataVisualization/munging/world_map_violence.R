...
df <- read.csv("DataAnalysis/Datasets/political_violence_events_by_country.csv")

ui <- fluidPage(
  titlePanel("Political Violence Across the World"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output) {
  ...
  output$mapPlot <- renderPlotly({
    ...
    # Create the plot
    violence.plot <- ggplot(world.map, aes(long, lat, group = group)) +
      geom_polygon(aes(fill = Events, text = Country), color = 'black', size = 0.2) +
      coord_quickmap() +
      theme_void() +
      scale_fill_viridis(name = "Index") +
      theme(legend.position = 'bottom') +
      labs(title = "Political Violence Across the World") +
      ggeasy::easy_center_title()
    
    ggplotly(demonstrations.plot, tooltip = c("text", "violence.plot"))
  })
}
